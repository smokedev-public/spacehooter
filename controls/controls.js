$(function() {
	...
	function joinGame() {
		...
		idRocket = Math.floor(Math.random() * (900000000000 - 2)) + 2;

		recordName = 'player/' + idRocket;
		...
		ds.record.getRecord( recordName ).whenReady(function( record ) {
			...
			record.once( 'delete',  function() {
				...
				// client goes offline)
				ds.event.unsubscribe( 'status/' + name + '/' + idRocket);
			});
			...
			ds.event.subscribe( 'status/' + name + '/' + idRocket , function(){});
